...
<label class="input {{ $errors->has($field->name()) ? 'state-error' : '' }}">
    <div class="input-group">
        <input data-placement="bottomLeft" class="form-control icp icp-auto" value="{{ $field->oldOrDefault() }}" name="{{ $field->name() }}" type="text" readonly="readonly"/>
        <span class="input-group-addon">
            <i class="fa {{ $field->oldOrDefault() }}"></i>
        </span>
    </div>
</label>

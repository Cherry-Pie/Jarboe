...
@if ($field->isTranslatable())
    @foreach ($field->getLocales() as $locale => $title)
        <div class="locale-field locale-tab-{{ $locale }} locale-field-{{ $field->name() }} locale-field-{{ $field->name() }}-{{ $locale }}"
            ...
            <textarea class="summernote-{{ $field->name() }}-{{ $locale }}-content" name="{{ $field->name() }}[{{ $locale }}]" style="display: none;">{!! $field->oldOrDefault($locale) !!}</textarea>
...
@else
    ...
    <textarea class="summernote-{{ $field->name() }}-default-content" name="{{ $field->name() }}" style="display: none;">{!! $field->oldOrDefault() !!}</textarea>
    @include('jarboe::crud.fields.wysiwyg.summernote.inc.error_messages', [
